	...
	public function save (){
		...
		if(!empty($_POST)) {
			...
			if(isset($data['system@id'])){
				$systeminfo['systemid']=USER::$get['system@id'];
			}
			error_log(">>>>>>>>>>>>>>>>>>>>>>>>system@id:>>>>>>>>>> ".$get['system@id']);   
			// $systeminfo['systemid'] = $get['system@id'];
			// if(isset($data['systemid'])){

			// 	$systeminfo['systemid'] = USER :: get('system@id');
			// 	error_log(">>>>>>>>>>>>>>>>>>>>>>>>$systeminfo['systemid']: ".$systeminfo['systemid']); 
			// }
			//$systeminfo['sysadd'] = defined('IN_ADMIN') ? 1 : 0;
			error_log(">>>>>>>>>>>>>>>>>>>>>>>>V('r:modelid'): ".V('r:modelid'));        //将错误消息写入到操作系统日志中  
			...
			if($id > 0){
				...
				// echo("000");
				// 主表
				error_log(">>>>>>>>>>>>>>>>>>>>>>>>tablename: ".$tablename.'_data');        //将错误消息写入到操作系统日志中  
				error_log(">>>>>>>>>>>>>>>>>>>>>>>>modelinfo: ".$modelinfo);        //将错误消息写入到操作系统日志中  
				foreach($systeminfo as $color)
				{
					error_log('>>>>>>>>>>systeminfo : '.$color);
					// echo($color);
				}
				// foreach($modelinfo as $color)
				// {
				// 		error_log('>>>>>>>>>>modelinfo  : '.$color);
				// }
