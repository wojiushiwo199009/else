 ...
 */		
	 ...
	 * @return 无返回值
	 */
	static function addPreDoAction($doRoute, $type, $args=array(), $except='') {
	 ...
	 * @return 无返回值
	 */
	static function redirect($mRoute,$type=1){
		//if(ENTRY_SCRIPT_NAME=='wap') {}
		switch ($type){
			case 1:
				APP::M($mRoute);
...
/*
		...
		#echo $type;die;
		#echo $f[$type];die;
		$f['tpl'] = str_replace("/../","/",$f['tpl']);
		$f['tpl'] = str_replace("//","/",$f['tpl']);
		$f['tpl'] = str_replace("application/templates","templates",$f['tpl']);
		if ( !isset($f[$type]) ){
